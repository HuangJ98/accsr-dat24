...
Common Voice dataset from Mozilla's website. This script extracts the data of the
...
def process_text(text, alphabet):
    ...
    is in the alphabet. Before removing non-alphabet characters, check that no tab
    is present in the text, as it is used as a separator in the TSV file. If a tab is
    found, remove all characters after it, and also the tab itself.
    """
    if "\t" in text:
...
def get_accents(cv_loc, mapping, acc_folder, alphabet, root_folder):
    ...
    df.rename(
        columns={"path": "audio_filepath", "sentence": "text"}, inplace=True,
    )
...
def split_data(
    ...
    for lang in seen:  # split seen data into train and test dicts
        split(
            lang, json.load(open(f"{acc_folder}/{lang}.json")), ratio, writers, shuffle,
        )
...
if __name__ == "__main__":
    logging.basicConfig(
        filename=f"logs/create_dataset/{int(now())}.log",
        level=logging.INFO,
        format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    )
    cv_loc = "/Users/cafr02/datasets/cv-corpus-10.0-2022-07-04/"
